...
class ToursListTest extends TestCase
{
    ...
    {
        $travel = Travel::factory()->create();
        $tour = Tour::factory()->create(['travel_id' => $travel->id, 'price' => 123.68,]);

        $response = $this->get('/api/v1/travels/'.$travel->slug.'/tours');

        $response->assertStatus(200);
        $response->assertJsonCount(1, 'data');
        $response->assertJsonFragment(['price' => '123.68']);
    }
    ...
    public function test_tours_list_filters_by_price_correctly()
    {
        ...
        $response = $this->get($endpoint.'?priceTo=800');
        $response->assertJsonCount(2, 'data');
        $response->assertJsonFragment(['id' => $cheapTour->id]);
        $response->assertJsonFragment(['id' => $expensiveTour->id]);
        ...
        $response = $this->get($endpoint.'?priceFrom=250&priceTo=800');
        $response->assertJsonCount(1, 'data');
        $response->assertJsonMissing(['id' => $cheapTour->id]);
        $response->assertJsonFragment(['id' => $expensiveTour->id]);
    ...
    public function test_tour_returns_validation_errors()
    {
        $travel = Travel::factory()->create();
